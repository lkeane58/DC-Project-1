...
import { ArrowRight, BookOpen, Compass, GraduationCap, PlusCircle, Search, Sparkles, Users } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] flex flex-col font-sans">
      ...
      {/* Main Container */}
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
        {/* TAB 1: SMART MATCH */}
        {activeTab === "match" && (
          <div className="space-y-7">
            <section className="grid overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)] lg:grid-cols-[1.35fr_0.65fr]">
              <div className="p-6 sm:p-8">
                <p className="text-xs font-bold uppercase tracking-[0.15em] text-jhu-heritage">Fall 2026 · Homewood</p>
                <h1 className="mt-3 max-w-xl text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Make studying less solitary.</h1>
                <p className="mt-3 max-w-xl text-base leading-7 text-slate-600">Choose your courses, discover student-created groups, and keep your meetings organized in one focused place.</p>
                <div className="mt-6 flex flex-wrap gap-3">
                  <button onClick={() => setIsCreateOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-jhu-heritage px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-jhu-dark">Create a group <ArrowRight className="h-4 w-4" /></button>
                  <button onClick={() => setActiveTab("explore")} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Browse groups</button>
                </div>
              </div>
              <div className="border-t border-slate-200 bg-slate-50 p-6 lg:border-l lg:border-t-0 sm:p-8">
                <p className="text-sm font-semibold text-slate-900">A simple way to begin</p>
                <ol className="mt-4 space-y-4 text-sm text-slate-600">
                  <li className="flex gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-jhu-heritage text-xs font-bold text-white">1</span><span><strong className="text-slate-800">Add courses</strong><br />Build a list from the JHU catalogue.</span></li>
                  <li className="flex gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border border-slate-300 bg-white text-xs font-bold text-slate-700">2</span><span><strong className="text-slate-800">Find a group</strong><br />See groups as students publish them.</span></li>
                  <li className="flex gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border border-slate-300 bg-white text-xs font-bold text-slate-700">3</span><span><strong className="text-slate-800">Meet with purpose</strong><br />Use the group’s real location and link.</span></li>
                </ol>
              </div>
            </section>
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 border-b border-slate-200 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Suggestions are based on your courses and study preferences.
                </p>
              </div>

              <button
                onClick={() => setIsProfileOpen(true)}
                className="text-sm font-semibold text-jhu-heritage hover:underline flex items-center space-x-1 self-start sm:self-auto"
              >
            ...
            {rankedMatches.length === 0 && (
              <div className="text-center py-14 bg-white rounded-2xl border border-slate-200 p-8 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                <span className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-blue-50"><Users className="w-6 h-6 text-jhu-heritage" /></span>
                <h3 className="text-lg font-bold text-slate-900">
                  No groups have been created yet
                </h3>
                <p className="text-sm leading-6 text-slate-500 max-w-md mx-auto mt-2 mb-5">
                  Start the first group for one of your courses. Joining becomes
                  available as soon as students publish groups.
                </p>
                <button
                  onClick={() => setIsCreateOpen(true)}
                  className="inline-flex items-center space-x-1.5 bg-jhu-heritage text-white text-sm font-semibold py-2.5 px-4 rounded-xl shadow-sm hover:bg-jhu-dark transition-colors"
                >
